...
class NotificationService {
  ...
  final scheduledChanelDescription =
      'Notifications scheduled for a period of time';
  ...
  Future<void> scheduledNotificationRepeat(
  ...
  TZDateTime _nextInstanceNotificationTime(DateTime? lastTimeStamp) {
    ...
    TimeOfDay defaultTime =
        SharedPreferencesController.getCurrentNotificationTime();
    ...
    TZDateTime scheduledDate = TZDateTime(local, planned.year, planned.month,
        planned.day, defaultTime.hour, defaultTime.minute);

    return scheduledDate;
  }

  Future<ScheduledNotificationModel> getScheduledNotificationFromEvent(
      EventsModel event) async {
    // Receive document data
    DocumentSnapshot snapshot = await PlantService.getPlantSnapshot(
        event.plantId, event.gardenId, event.userId);
    ...
    ScheduledNotificationModel notification = ScheduledNotificationModel(
        ...
        dateTimeComponent:
            PeriodsHelper.getDateTimeComponentsFromPeriod(event.period),
